...
lcz_anomaly <- function(x,
  ...
  if (is.null(by)) {
    ...
    final_graph <-
      ...
      ggplot2::geom_text(data = base::subset(anomaly_cal, anomaly >0), ggplot2::aes(label = round(anomaly, 1)), hjust = -0.1, size = 4, fontface = "bold") +
      ggplot2::geom_text(data = base::subset(anomaly_cal, anomaly <0), ggplot2::aes(label = round(anomaly, 1)), hjust = 1.1, size = 4, fontface = "bold") +
