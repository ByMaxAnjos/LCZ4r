...
We calculated monthly mean temperature anomalies for May 2023. 

```{r eval=FALSE, message=FALSE, warning=FALSE}
# Calculate monthly anomalies
cws_anomaly <- lcz_anomaly(lcz_map,data_frame = data_qc,
                var = "ta_int", station_id = "p_id",
                time.freq = "month",
                extract.method = "two.step",
                year = 2023, month = 5)
...
```{r echo=F, out.width = '100%', fig.align='center'}
knitr::include_graphics("fig_local_modeling_crows_3.png")
```

In this case, stations classified as LCZ 17 (Water) were excluded due to potential misclassification issues. As shown in the figure, we removed stations identified as LCZ 17 (Water bodies), specifically stations 55280926 and 363960316.

```{r eval=FALSE, message=FALSE, warning=FALSE}

#Check and delete stations
new_data_qc <- data_qc %>% 
  mutate(p_id = as.factor(p_id)) %>% 
  filter(!p_id %in% c("55280926", "363960316"))

# Calculate daily anomalies
cws_anomaly <- lcz_anomaly(lcz_map,data_frame = new_data_qc,
                var = "ta_int", station_id = "p_id",
                time.freq = "day",
                extract.method = "two.step",
                year = 2023, month = 5, day =1,
...
```{r echo=F, out.width = '100%', fig.align='center'}
knitr::include_graphics("fig_local_modeling_crows_4.png")
```
...
cws_uhi <- lcz_uhi_intensity(lcz_map, data_frame = new_data_qc,
                var = "ta_int", station_id = "p_id",
                time.freq = "hour",
                extract.method = "two.step",
                year = 2023, month = 5,
...
We modeled the UHI for May 13, 2023, at 16:00, using spatial interpolation.

```{r eval=FALSE, message=FALSE, warning=FALSE}
# Map air temperatures for 2023-05-13 at 16h.
my_interp_map <- lcz_interp_map(
  lcz_map,
  data_frame = new_data_qc,
  var = "ta_int",
  station_id = "p_id",
  sp.res = 100,
  tp.res = "hour",
...
### Evaluate UHI map interpolation 
