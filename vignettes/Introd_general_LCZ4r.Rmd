...
## Obtain and visualize the LCZ map {.tabset .tabset-fade .tabset-pills}
...
-   1\. As an alternative, users can download their own LCZ map from [WUDAPT](https://www.wudapt.org/) or [LCZ generator application](https://lcz-generator.rub.de/submissions) and integrate them into the LCZ4r system using the lcz_get_map2(LCZmap, city = "your city").
...
-   3\. Custom ROI: Utilize the lcz_get_map() function along with a shapefile for your ROI by coding lcz_get_map(roi="your_shp"). Refer to the R package [sf](https://r-spatial.github.io/sf/) documentation for guidance on loading the shapefile into R. \## Calculate LCZ areas
